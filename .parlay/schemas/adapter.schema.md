...
The design system section is a structured inventory of where each category of design decisions comes from. It tells the agent: for colors, use framework tokens; for icons, the framework does not define them, so use sensible defaults.

Each category has three fields:

| Field | Required | Description |
|---|---|---|
| `source` | Yes | Where values come from: `framework` (use built-in tokens) or `not-defined` (agent uses sensible defaults) |
| `format` | Yes (when source is `framework`) | How to use the values — token names, component APIs, import paths |
...
When `source: framework`, the agent uses the framework's token system and never hardcodes values. When `source: not-defined`, the agent uses its judgment.
...
| `adapter-design-system-source-unknown` | A `design-system:` category's `source:` is outside `{framework, not-defined}`. |
...
| Design system | Parlay (ships defaults for known frameworks) | Team (marks source per category) | Framework upgrade |
