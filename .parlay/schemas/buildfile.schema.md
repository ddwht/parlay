...
**Status: the v2 (multi-target) shape below is the design target, not yet the accepted shape.** The validator currently accepts only the v1 single-adapter shape — a buildfile written to the v2 structure documented here is rejected with `schema-validation-failed: buildfile missing 'adapter' field`. Until multi-target validation lands, author v1: top-level `schema_version: 1` plus `adapter:`, as documented in the [Appendix](#appendix-legacy-v1-buildfile-shape-frozen).

This section previously claimed v2 was "primary … every project, including presentation-only ones, is described in this shape from here on." That statement did not match the implementation, and following it produced a buildfile the CLI refused — a cost paid on every build until an agent reverse-engineered the validator by trial. New multi-target content still lands in the shape below; single-target projects use the appendix shape until the validator catches up.
...
## Structure (v2 multi-target — the design target, NOT accepted by the validator)
...
**To author a buildfile that validates, use [the accepted shape](#appendix-legacy-v1-buildfile-shape-frozen).** Everything below this heading describes fields, rules, and diagnostics that apply to both shapes except where the multi-target `targets:` block is involved; the sections on `targets.<kind>:`, adapter-set resolution, and per-target projection are v2-only and therefore inert today.
