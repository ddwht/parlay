...
**Enforcement layer: skill-mechanical, not CLI.** This gate is executed by the `generate-code` skill itself (its freshness-gate step, alongside the `build-feature` skill's "Emit the `source-signatures:` section" step that writes the recorded values) — there is no `parlay` CLI subcommand that reads or checks `source-signatures:`. "Mechanical" here means the same thing it means for the buildfile's determinism contract elsewhere in this doc: a fixed procedure the agent executes exactly, with no AI judgment call in the comparison itself, not that a Go binary performs it. This is a deliberate, related-but-distinct mechanism from the Go-side advisory freshness signal — `HashedSources` in `.parlay/build/<feature>/.baseline.yaml`, which feeds `parlay diff`'s component-level stable/dirty/removed classification and is being extended to cover `capabilities`/`infrastructure`/`surface.yaml` in separate work. Don't conflate the two or "unify" one away against the other without understanding both: `source-signatures:` is a hard per-feature gate enforced by the skill at codegen time (refuses to emit anything on mismatch); `HashedSources` is an advisory, component-level signal that informs `parlay diff`'s regenerate/preserve recommendation but never blocks a run on its own.
