...
26 schemas, 211 error codes, 434 KB of source.
...
| `activity-undeclared` | `next-activity-review` | There is no file. |
...
| `amendment-affects-unresolved` | An `affects:` ref names an operation/fragment/entity that does not exist in the referenced feature's contract artifacts, and the record declaring it is not… |
| `amendment-authority-unreadable` | The feature's `.baseline.yaml` exists but its applied-authority record could not be read, so no amendment can be shown applied. |
| `amendment-compaction-incomplete` | A compaction of this feature was interrupted and its journal is still in place, so the ledger may be half-moved. |
...
| `amendment-intent-transition-malformed` | An `amends_intents:` entry has an unknown mode, authors the read-only `legacy_supersession`, supplies no new goal for a mode that requires one, supplies text… |
...
| `amendment-retirement-names-retired-intent` | It names an already-retired intent. |
| `amendment-retirement-not-terminal` | More than one record carries the marker, or a record follows the retirement. |
...
| `amendment-transition-receipt-invalid` | The receipt exists but does not describe this record: wrong ceremony mode, a promise approved twice or not at all, a closure value that contradicts the mode,… |
| `amendment-transition-receipt-missing` | A trusted-applied `amends_intents:` record has no transition receipt in the baseline. |
| `feature-retirement-has-output` | The feature still has contract artifacts, a buildfile or testcases. |
| `feature-retirement-scan-incomplete` | An artifact could not be read or parsed, so nothing was established. |
...
| Code | Fires when |
|---|---|
| `annotation-after-close` | An entry directly follows a `close` at the same column. |
| `annotation-in-applied-record` | An annotation inside an amendment the baseline has already applied. |
| `annotation-in-block-scalar` | The sigil appears inside a YAML block scalar, where it is content. |
| `annotation-inline` | The annotation shares a line with content. |
| `annotation-malformed` | A comment opens with the sigil but does not match the grammar — no colon, an illegal handle, an unterminated quote, `section` in YAML, a tab-indented YAML… |
| `annotation-phrase-not-found` | The quoted phrase does not occur in the anchored unit. |
...
| `annotation-word-unknown` | A word between the handle and the colon is neither a kind in `{do, ask, close, done, answer, declined}` nor the scope word `section`, or a kind or the scope… |
| `answered-annotations` | The feature has answered threads. |
| `closed-annotations` | Closed threads are still in the files. |
| `open-annotations` | The feature has at least one `open` thread. |
...
| `backlog-item-stale` | Open and undecided for 90 days or more |
...
- `coverage-decisions.schema.md` — `.parlay/build/<feature>/coverage-decisions.yaml` — the judgments a person has
- `criteria-authority.schema.md` — `.parlay/build/<feature>/criteria-authority.yaml` — who accepted the standard a
