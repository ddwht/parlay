...
`schema_version: 3` is the current shape: a `kind:` discriminator over the closed set `{presentation, operation}`, `file:` on every suite, and `criterion: {ref, text}` on every case.
