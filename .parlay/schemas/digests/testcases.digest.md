...
The suite is `kind: presentation`, `scope: route`, and marked `origin: page-assembly`. It derives entirely from the RESOLVED composed page — supersession applied project-wide, then filtered to this feature — and carries no hand-authored expectations.

**`origin:` is what marks a suite derived, not `scope:`.** `scope: route` is organizational and a route suite may hold real acceptance tests, so exempting `scope: route` from criterion accounting would silently drop genuine coverage.

**A derived case discharges no criterion.** Instead of `criterion: {ref, text}` it carries:

```yaml
derivation:
  kind: page-assembly
  page: <page name>
  subject: <component slug>
  assertion: mounted | hit-reachable | not-hit-reachable
```

`(page, subject, assertion)` is the case's identity — what the emitter writes, what the validator diffs, and what duplicate detection keys on, so a rename or a step reorder cannot evade it. Derived cases are excluded from criterion coverage; `testcases-case-criterion-missing` does not apply to them, and a `criterion:` on one is `assembly-case-cites-criterion` because it claims coverage the case does not provide.

**Unsupported assertions are capability debt, not weakened tests.** Where the adapter cannot execute an assertion it is recorded in `pending_assertions` with the capability it needs, and reported as `assembly-assertion-unsupported` (warning). It is NOT stamped `coverage: state-only`: that field records a weakened observation of a contract criterion, and a composition fact has none to weaken. Stamping it produced `criterion-observed-weakly` with an empty ref and text, which `record-exception` refuses — a blocker no human or machine could discharge.

`mounted` needs the adapter's `render-support: mounted`; both reachability assertions need `hit-testing-support: true`, a separate term because `render-support` is the closed set {mounted, output, content} and has no vocabulary for pointer reachability at all.

---

<!-- parlay-extends: parlay-tool/multi-adapter/testcases-v2 -->

`schema_version: 3` is the current shape: a `kind:` discriminator over the closed set `{presentation, operation}`, `file:` on every suite, and `criterion: {ref, text}` on every **contract** case. Cases in an `origin: page-assembly` suite are the one exception — they carry `derivation:` instead and are excluded from criterion accounting (see § Derived assembly suite). Without that exception the two sections contradict each other, and the producer emits an illegal file whichever it obeys.

**The version is the graduation trigger.** The criterion diagnostics — uncovered, citation drift, missing text, unknown ref, cross-kind unexercised, and the missing `file:` — are warnings on a file below 3 and **errors** at 3, because a file declaring the current shape is one where those facts could have been recorded. Policy: **regenerate**. An older version is an instruction to re-run `/parlay-build-feature`, not something to migrate in place.
...
| `testcases-cross-kind-criterion-unexercised` | Warning below `schema_version: 3`, error at 3 or above. A presentation case cites an operation's criterion but no step targets that operation — the ref is standing in for a display criterion rather than discharging a contract one. |
| `verify-criterion-duplicate` (warning) | A contract entry declares the same `verify:` bullet twice. Two identical bullets cannot be discharged separately; fix the contract rather than indexing around them. **Deliberately outside the revision split**: the defect is in the contract, not in the file being validated, so it must not fail a file whose author may not own the problem. |
