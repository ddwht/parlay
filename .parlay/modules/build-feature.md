...
9. **Generate testcases.yaml** at `.parlay/build/{feature}/testcases.yaml` (tool-internal — drives cross-validation and feeds spec generation, never handed off to engineering):
   ...
   - **On each case, state why it exists and what it touches** (see `testcases.schema.md` § Criterion-driven cases):
     ...
     - `coverage:` — `full` by default. Stamp `state-only` when a display-shaped criterion ("the viewport shows the mesh") has no assertion vocabulary and you compiled it down to a store-level assertion ("the store holds the mesh"). This records the downgrade so the coverage reviewer sees a weaker claim rather than a silent one. Do not stamp it to dodge a hard assertion — it is an admission, not an escape hatch.
