...
```
parlay internal gate @{feature} --stage build
```
...
5. **Read feature files**:
   ...
   - `spec/intents/{feature}/domain-model.yaml` (if exists — the feature contribution)
   - A feature needs at least one of `surface.yaml`, `capabilities.yaml`, or `infrastructure.md` — the four spec artifacts are co-equal — `surface.yaml` (or legacy `surface.md`), `capabilities.yaml`, `infrastructure.md`, and the project's `domain-model.yaml` — none is a stand-in for another. Features that are purely user-facing have only `surface.yaml`; features that expose operation-shaped backend behavior have `capabilities.yaml`; features that introduce architectural prose (boundaries, probes, allowlists, dependency pins) have `infrastructure.md`; many features have several of these in combination.
...
8. **Generate buildfile.yaml** at `.parlay/build/{feature}/buildfile.yaml` (tool-internal location — designer never sees this):
   ...
   - **When infrastructure.md exists** — translate each infrastructure fragment into a `cross-cutting:` entry via the **adapter bridge**. Infrastructure fragments are framework-agnostic (Affects / Behavior / Invariants); the buildfile entry is framework-specific. The translation is a resolution step, not a 1:1 field rename:
     ...
     - If a feature has both `surface.yaml` and `infrastructure.md`, the buildfile has both `components:` (from surface) AND `cross-cutting:` (from infrastructure).
   ...
   - **Preserve the `decisions:` section verbatim.** Implementation-level judgment calls codegen recorded live in `decisions:` (see buildfile.schema.md). Carry the existing block through unchanged from the prior buildfile — it is accumulated judgment, not build-time state, and `generate-code` is its only author. Do NOT invent, rewrite, or prune entries here; a decision retires only when a later one names it in `supersedes:` or its `obsolete-when:` condition holds, and that is codegen's call to make, not this phase's. When no prior buildfile exists, omit the section.
...
9. **Generate testcases.yaml** at `.parlay/build/{feature}/testcases.yaml` (tool-internal — drives cross-validation and feeds spec generation, never handed off to engineering):
   - **Set `schema_version: 3`.** This is the shape carrying criterion identity
     on every case and `file:` on every suite, and declaring it is what makes
     the criterion checks BLOCK rather than warn. A file that omits it, or
     declares less, reads as one written before those fields existed — which is
     the whole reason those checks were warnings — so a current build that
     leaves it off is asking to be graded by the rules for legacy artifacts.
   - **One suite per component (grouping), but cases come 1:1 from `verify:` bullets (coverage).** A suite is where a component's cases live; it is not the unit coverage counts. Do not invent cases to fill a suite, and do not judge a suite done by "the component is exercised" — judge it by "every `verify:` criterion on this component's contract has a case that discharges it". Enumerating components and stopping when each has a suite is what let vacuous suites satisfy the gate; enumerating criteria is what this replaces it with.
   ...
   - **Derive test assertions from the contract artifacts' `verify:` fields first** — the operation's `verify:` in capabilities.yaml for operation suites, the fragment's `verify:` in surface.yaml for component suites. There is no intent-bullet fallback since v0.3: never reach into intents, whose bullets are history and may predate amendments the contract has absorbed.
   - **A source entry with no `verify:` at all: omit the suite, do not invent criteria for it.** Emit no cases for that entry, record the omission in your summary naming the entry, and continue with the rest of the build. If omitting leaves the feature with no meaningful presentation build at all — every fragment vacant — say *that*, plainly, as the outcome. Writing criterion-less cases instead is the failure this rule exists to prevent: they cite nothing, discharge nothing, and report as a testing defect one phase later while the actual hole is in the artifacts. `parlay migrate-verify` relocates stragglers for a project that predates relocation, but it routes to operations first, so it will not fill a fragment vacancy unless run with `--fragments` — and that flag only seeds a draft a human must review, since it cannot tell a UI claim from a contract one. Authoring the presentation claims (see `/parlay-create-artifacts` § Routing acceptance criteria) is the real fix. A *wholly* vacant surface no longer reaches you: the build gate blocks it. What reaches you is partial vacancy, which is why this rule is per-entry rather than per-feature.
   - **On each case, state why it exists and what it touches** (see `testcases.schema.md` § Criterion-driven cases):
     - `criterion: {ref, text}` — the criterion this case discharges. `ref` names the contract entry in `@{feature}/{kind}:{name}` form; `text` is **the bullet's own wording, copied verbatim** from that entry's `verify:` list. Both halves are the identity: coverage is counted per bullet, so a `ref` with no `text` cannot say which of the entry's criteria it covers (`testcases-criterion-text-missing`), and a text matching none of them is `testcases-criterion-text-drift`. A case without a `criterion:` at all draws `testcases-case-criterion-missing`, which is an **error** in the `schema_version: 3` file you are writing — it warns only on files that predate the field. The whole point of criterion-driven derivation is that you filled it in as you wrote the case. **This rule is about CONTRACT cases only.** A case in an `origin: page-assembly` suite carries `derivation:` instead of `criterion:` and is exempt — see the assembly bullet below. Requiring a citation there is what made the two instructions contradict each other, and the exemption is the resolution, not a loophole.
     - **Citing across kinds.** A presentation case may discharge an *operation's* criterion when it genuinely invokes that operation — give one step the operation's ref as its `target:`, or you draw `testcases-cross-kind-criterion-unexercised`. Do not reach for an operation criterion because the fragment has none: that is the substitution the rule exists to stop, and it compiles a display claim to a store assertion without the `coverage: state-only` stamp that would have recorded the downgrade. A fragment with no criteria means the suite is omitted (above), not that its cases borrow the backend's.
     - `exercises:` — the targets the case's steps must mutate. Every target you list here must appear as a step `target:`, or the case is `testcases-case-vacuous`. This is the guard against writing a case that satisfies the gate while asserting nothing — you cannot claim to exercise something you never touch.
     - `observes:` — the targets the case's expectations read. Every `verify:` step must read a target listed here, or the case is `testcases-case-claims-unmet`. Declare what the case asserts on, then let the validator hold the assertions to it.
     - `coverage:` — `full` by default. Stamp `state-only` when a display-shaped criterion ("the viewport shows the mesh") compiles down to a store-level assertion ("the store holds the mesh") because the adapter cannot deliver the `appears` level it needs. This records the downgrade so the coverage reviewer sees a weaker claim rather than a silent one. Do not stamp it to dodge a hard assertion — it is an admission, not an escape hatch.
   - **Compile display criteria to `appears`, capability permitting** (see `testcases.schema.md` § The `appears` step). A criterion about what the user *sees* — a rendered mesh, a mounted panel, a populated list — wants an `appears` step at `mounted`, `output`, or `content`, not a store read. Check the presentation adapter's `render-support:` (adapter.schema.md): if it lists the level the criterion needs, emit the `appears` step (`action: appears`, `target:` the component, `value:` the level) and leave `coverage: full`. If it does not — every adapter without a `render-support:` block, which is today's default — compile the criterion to the store-level assertion and stamp `coverage: state-only`. Do not invent an `appears` step against an adapter that declares no support for its level; it will not be runnable. When the adapter later lists the level, the next rebuild lifts the stamp automatically, because the criterion already records what it needs.
   - **Do NOT hand-write the per-page assembly suite.** It is WRITTEN by the tool. Run:

     ```
     parlay internal emit-assembly @{feature} --write
     ```

     This merges the derived suites into `testcases.yaml`, replacing any existing `origin: page-assembly` suites and leaving every authored suite untouched. Readiness then validates the result by diffing it against the same derivation — including each case's steps, not merely its label, because a case with the right `derivation:` block and empty or unrelated steps asserts nothing while counting as checked. Do not compose the suite yourself, do not retype what the writer produced, and do not edit it afterwards.

     This bullet used to tell you to derive the suite by hand "with no assertions you invent", six lines after the rule above requiring `criterion: {ref, text}` on every case. Those two instructions cannot both be obeyed: an assembly assertion is a composition fact — *this component reached the renderer* — and there is no contract criterion for which that is the evidence. Every attempt to satisfy both produced an illegal file, and when a criterion was cited anyway it was cited falsely (a case asserting four components mounted, citing a criterion about which columns a table shows).

     So the resolution is explicit: **an assembly case discharges no criterion.** It carries `derivation: {kind: page-assembly, page, subject, assertion}` instead of `criterion:`, and it is excluded from criterion coverage rather than counted toward it. `testcases-case-criterion-missing` does not apply to it. Neither does `coverage: state-only` — that field records a weakened observation of a *contract* criterion, and a composition fact has none to weaken; stamping it produced a blocker carrying an empty ref that `record-exception` refuses, undischargeable by anyone.

     Where the adapter cannot execute an assertion (today: all of them, since no adapter declares `render-support:` or `hit-testing-support:`), the emitter records it as capability DEBT in `pending_assertions`, reported as a warning. It is not a weakened test and not a missing one — it is an assertion the framework cannot yet make.
...
10.9 **Prove the tests discharge the standard, before handing off.** Run:

    ```
    parlay internal check-testcases-ready @{feature}
    ```

    This is the code boundary's OWN readiness computation, called here rather than one phase later. It exits non-zero when a criterion has no case discharging it, when a case cites something that does not exist, or when the derived assembly suite disagrees with the composed page.

    **Every blocker it reports is yours to fix, here, by deriving again.** They are not findings to hand downstream: this is the phase that writes the cases, and uncovered criteria are incomplete output from it. Re-derive against the exact criteria it names and run it again.

    Do **not** discharge a blocker by borrowing a criterion for a case that does not observe it, by stamping `coverage: state-only`, or by recording a coverage exception. The first two manufacture false coverage; the third is a person's judgment to make about a criterion that genuinely needs no test, not a way for this phase to clear its own incomplete work.

    If blockers remain after re-deriving, raise a `failure` decision request rather than handing off. A run that stops here has lost one phase; a run that hands off regardless is refused at the code boundary having spent two.
...
- `no-surface` — surface.yaml doesn't exist. Suggest running `/parlay-create-artifacts @{feature}` first.
