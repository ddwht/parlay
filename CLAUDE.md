...
- `/parlay-resolve` — Act on the review comments people wrote in the files — one thread at a time, through the route each file's governance requires
...
- **Saying something is wrong, in any of these files**: write a review comment where the problem is — `<!-- @you: ... -->` in Markdown, `# @you: ...` in YAML, on its own line directly BELOW the text it is about. /parlay-resolve then routes it through whatever that file's governance requires. This is the alternative to editing, not a way around it: a comment in a frozen founding document is legal because the ledger hashes PARSED content and no parser reads comments, so it does not look like an edit to a promise — but comment bytes do move the byte-based build signatures, which is exactly why every review thread blocks the build and code boundaries until it is closed and swept. An applied amendment refuses annotations outright, for that same reason.
- **spec/intents/<feature>/amendments/** (designer-authored, append-only): NNN-<slug>.md — one file per change, written once and NEVER edited **once applied**; a correction is a new amendment naming the old one in supersedes:. An unapplied record has no recorded hash and is still a proposal in its review window, so it may be corrected in place, preserving its slug and sequence. Compaction may move files to amendments/archive/, never delete them.
