...
// The apply path writes through the same serializer the editor's save uses,
// so the bytes a CLI accept produces are the bytes an editor accept would.
func TestApplyContributionWritesThroughTheEditorsSavePath(t *testing.T) {
	root := t.TempDir()
