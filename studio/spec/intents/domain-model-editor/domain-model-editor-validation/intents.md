...
**Context**: The domain-model schema pins a closed vocabulary of validation codes: `missing-schema-version`, `schema-version-newer-than-binary`, `schema-version-unreachable`, `field-type-outside-closed-set`, `enum-tone-outside-closed-set`, `undeclared-entity-reference`, `relationship-cardinality-unknown`, `operation-input-field-not-found`, and the authoring-mode warning `domain-operations-deprecated`. Core owns these rules in one place — `ValidateDomainModelStructured`, which returns one finding per violation — sealed under `core/internal/` and reached through the CLI's `validate` command. Studio is a separate Go module in a different namespace and **deliberately does not import Core** (a guard test enforces the no-Core-import boundary), so the editor reaches Core's validator **out of process**: it invokes the `parlay validate --type domain-model --json` CLI mode with the draft on stdin and parses the returned findings. This is stronger than sharing source — the editor and the build path run the *same binary*, so they cannot drift even in principle, which is why in-process import was rejected. The `--json` structured-output mode, the per-finding element path, and the emission of `domain-operations-deprecated` (which Core declares in its severity table but does not yet raise) are all owned by the Core-side feature `studio-support/structured-domain-model-validation`, which this feature depends on and consumes verbatim. The editor's closed-vocabulary forms make several codes unrepresentable from form input (that was the MVP's point), but the validator still sees them: hand-edited files arrive through load, and cross-element states a form can't see locally (a ref whose target was deleted out from under it in a hand-edit) are exactly what deep validation exists to catch. Studio-side, the harness's `validation-failed` envelope carries `fields[]` — the natural transport, with each entry carrying the closed code and a path to the offending element.

**Action**: The Studio subsystem invokes `parlay validate --type domain-model --json`, feeding the in-memory draft (serialized to YAML) on stdin and parsing the emitted finding list — each finding carrying the closed code, an element path (e.g. `entities.Order.fields.status`, `relationships.customer-orders.to`), the schema's actionable message, and its fix text. It wraps this at `POST /api/domain-model/validate`, accepting a model draft and returning the findings as a `validation-failed`-shaped payload (200 with an empty list when clean — a finding list is a query result, not an error). Severity follows the schema's authoring-mode table: `domain-operations-deprecated` is a warning; everything else is an error. Studio adds no validation rule of its own; the structured output, element-path field, and deprecated-warning emission it relies on are delivered by the Core-side feature.

**Objects**: core-validate-json-cli-mode, validate-subprocess-invocation, validation-finding, closed-error-code, element-path, validate-endpoint, severity-classification, parity-test-suite

**Constraints**:
- The editor obtains findings only by invoking Core's `parlay validate --type domain-model --json` subprocess; no validation rule is reimplemented in Studio Go code or in the UI bundle's JavaScript, and Studio does not import Core as a library — client-side form guards (closed dropdowns, pickers) are input affordances, not the validator, and the server-side finding list is always authoritative
- Findings carry exactly the schema's closed error codes, unchanged, as emitted by Core; the editor never invents codes, renames codes, or adds Studio-only rules to the deep-validation pass
...
- Severity follows the schema's authoring-mode table: `domain-operations-deprecated` is a warning, all other codes are errors; the classification is Core's, surfaced through the CLI output, not recomputed in the UI
- The validate endpoint is a pure function over the submitted draft: it reads nothing from disk, mutates nothing, runs the validator against the draft bytes alone, and returns 200 with the finding list (empty when clean) — `validation-failed` at the HTTP-error level is reserved for malformed requests, not for a draft with findings
- A parity test suite in the studio tree runs a shared fixture corpus (one fixture per closed code, plus clean fixtures) through both `parlay validate --type domain-model --json` directly and the editor's endpoint and asserts identical finding sets — since both run the same binary, this guards the Studio wrapper (request shaping, stdin transport, path mapping) rather than re-testing Core's rules, and failing it blocks the build

**Verify**:
- A draft carrying a `field-type-outside-closed-set` violation POSTed to `/api/domain-model/validate` returns 200 with exactly that closed code and an element path resolving to the offending field (`entities.<E>.fields.<F>`)
- A clean draft returns 200 with an empty finding list — a finding list is a query result, so a draft with findings is never an HTTP error
- A shared fixture corpus (one fixture per closed code, plus clean fixtures) run through the endpoint and through `parlay validate --type domain-model --json` directly yields identical finding sets — the parity/drift alarm
- A draft with a populated deprecated `operations:` block returns a `domain-operations-deprecated` finding at warning severity while every other code is an error (severity taken from the schema's authoring-mode table, not recomputed in Studio)
- A whole-model violation (e.g. `missing-schema-version`) returns a finding carrying the distinguished top-level path, not a blank or fabricated element path
- A malformed request (unparseable draft bytes or a bad request envelope) returns `validation-failed` at the HTTP-error level, distinct from a 200 finding-list response for a well-formed but invalid draft
- The endpoint is side-effect-free: validating a draft writes nothing to disk and mutates nothing, and returns the same findings regardless of the on-disk model — it validates the submitted bytes alone

**Depends-on**: `studio-support/structured-domain-model-validation` (Core root) — provides the `--json` structured output mode on `parlay validate --type domain-model`, the element-path field on each finding, and the `domain-operations-deprecated` emission.
