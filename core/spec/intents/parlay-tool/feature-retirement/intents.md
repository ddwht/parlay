...
- YAML artifacts are read as parsed documents and only the values of reference-carrying keys are inspected, never raw lines. A folded or block scalar spreads one value across lines that do not begin with its key, and a flow mapping puts the key inline behind a brace; both are legal, and a line scan reports a clean result on either — the worst possible failure for a check whose whole job is to establish that nothing points here. Markdown artifacts are read by their own field grammar, and a ledger is read through the amendment loader rather than by recognizing the spellings of `affects:` entries.
- The scan is **fail-closed**. An artifact that is present but unreadable or unparseable, or a ledger that will not load, is recorded as a scan failure and refuses the retirement — a check whose whole job is to establish that nothing points here cannot report a clean result over a file it failed to read. An absent *optional* artifact is absence; everything else is "cannot tell", and cannot tell is not none.
- Some references are mapping **keys** rather than values — canonical operations are indexed by qualified ref — so the positions are named per artifact and per position, not as one list of field names that a key walk could never consult.
- References are matched against the feature's qualified and bare forms with a trailing delimiter, so a feature is not matched by a longer name beginning with it.
