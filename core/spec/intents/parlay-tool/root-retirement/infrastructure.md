...
- An acknowledged reference naming an artifact, a reference and a rationale stops that one finding blocking the retirement, and the preview lists it under its own heading with the reason given rather than dropping it silently.
- An acknowledgment matches only on an exact artifact and an exact reference; a different path, a different reference or a path that is merely a prefix of the finding's dismisses nothing.
- A finding nobody acknowledged still refuses the retirement, so the dismissal narrows nothing about what is reported.
- An acknowledgment carrying no path, no reference, no rationale, or a key the shape does not define, is refused like any other part of the record.
