...
The NAME is subject to the same rule as the path, because it is a path too. It is concatenated into the staging directory, the archive destination and the journal filename, and all three are then created, renamed and removed — so a name carrying a separator or a traversal segment addresses a location as effectively as a path does, and the destination rollback is a recursive delete. The name must therefore be a plain slug: lowercase alphanumerics in dash-separated words, nothing else. The shape is restricted rather than escaped, because escaping asks every future caller to remember to escape while a closed shape is checked once at the boundary and lets everything downstream treat the name as a single safe component.

Containment is judged before the act, and the act is then performed in a way that cannot be re-aimed after the judgment. A resolved path is a fact about a moment; between that moment and the deletion, an intermediate directory can be replaced with a link leading out of the project, and a deletion following the resolved path would follow it out. Re-checking would narrow that interval rather than close it. So every destructive step is performed through a handle rooted at the project root, which resolves each path component and acts in one sequence and refuses any component leading outside — there is no interval left to exploit. This governs every step that changes the filesystem, not only the deletions. A write is not safer than a delete here: the retirement record, the journal, and the rewritten root registration all land in locations that can be replaced with a link out of the project between one step and the next, and a path-based write would then create the file somewhere the project does not own while leaving the real location without it — which is precisely the half-retired state the ordering exists to prevent, reached by a different route. The archive's own copy writes into a staging directory the same run created, under a name already established as a single component.
**Invariants**:
...
- A registered root name that is not a plain slug refuses the run before any mutation, whatever its registered path says.
- A name carrying a separator, a traversal segment, a dot, whitespace or upper case is refused; the ordinary names this project produces are accepted.
- Every destructive step acts through a handle rooted at the project, so a path component replaced with a route out of the project after containment was judged is refused rather than followed, and content outside the project survives the attempt.
- The retirement record, the journal and the rewritten root registration are written through that same handle: a location swapped for a route out of the project refuses the write rather than landing outside, and nothing is created outside the project by the attempt.
- The removal of the journal is rooted on the same terms, so a swapped location cannot make the run delete a file the project does not own.
...
- An acknowledged reference naming an artifact, a position, a reference, a kind and a rationale stops that one finding blocking the retirement, and the preview lists it under its own heading with the reason given rather than dropping it silently.
- An acknowledgment matches only on the finding's full identity: a different path, position, reference or kind — or a path that is merely a prefix of the finding's — dismisses nothing, so a second occurrence of the same reference is a second judgment.
- A finding nobody acknowledged still refuses the retirement, so the dismissal narrows nothing about what is reported.
- An acknowledgment carrying no path, no position, no reference, no kind, no rationale, or a key the shape does not define, is refused like any other part of the record.
- An acknowledgment that matches no finding is reported as having matched none, so a misspelling is visible rather than looking like a refusal nobody can explain.
...
Two kinds of file inside the project tree are nonetheless not project content, and reading them produces findings nobody can act on. The first is a nested git checkout — a linked worktree, a submodule, a vendored clone — which is another commit's copy of the project rather than the project. Its markers name whatever the features were called at that commit, and they read exactly like live inbound references; worse, the operator cannot resolve them, because editing a checkout of an old commit into agreement with the present one is not a thing anyone should do. Any directory below the project root carrying a .git entry, file or directory, is skipped whole. The second is the operator's own disposition record, which necessarily enumerates every feature in the retiring root — that being what it is for — so scanning it reports the operator's answers back as evidence against them. Exactly the resolved path passed to the run is exempt, and nothing broader: another file that merely looks like a disposition record is scanned like anything else.

Beyond that, the sweep does not decide that a mention is only a mention, and it is not asked to. It reports, and a person judges. The judgment is recorded in the disposition record as an acknowledged reference naming one artifact and one reference exactly, with a rationale; the preflight subtracts findings that match both, lists them in the preview under their own heading with the reason given, and refuses on every finding nobody acknowledged. Fail-closed is unchanged — nothing is dismissed that a person has not named — and the human dismissal the operation always promised becomes something the operator can actually write down rather than something they achieve by deleting the sentence.
**Invariants**:
...
- A nested git checkout below the project root — marked by a .git file or directory, as a worktree, submodule or vendored clone is — is skipped whole: its contents are another commit's copy of the project, and neither its markers nor its instructions are findings.
- A skipped checkout is not a scan failure either, and re-home readiness is never judged against one.
- The disposition record passed to the run is exempt from the sweep, at exactly its resolved path; another file shaped like a disposition record is scanned like anything else.
...
Because a journal is found by scanning a location rather than being handed over, it is checked before it is acted on. What a resumed run does on its authority is delete a directory and remove a registration, with no preflight, no sweep and no disposition record — correctly, since the run that wrote the journal performed all of those.

It is tempting to describe those checks as establishing that the journal is genuine. They do not, and cannot, and saying otherwise would be a promise the operation cannot keep. The journal, the manifest, the retirement record and the manifest digest are all files, all writable by whoever runs the tool; anyone able to forge one can forge the set, consistently, and no comparison among them can tell the difference. Distinguishing a genuine artifact from a well-made forgery requires a trust anchor outside the repository — a signature, a key, an append-only log kept somewhere the forger cannot reach — and this operation has none. AUTHENTICITY IS THEREFORE OUT OF SCOPE. That residual is stated here rather than papered over, because a security property claimed and not held is worse than one never claimed.

What the checks do deliver is three things, and those three are worth having. The first is CONSISTENCY. Closed decoding refuses an unknown key rather than dropping it; the recorded root is a plain slug and equals the name the file is filed under; the recorded path is contained; the outstanding steps are a non-empty tail of the canonical order, which rules out unknown, duplicated, reordered and post-terminal steps together; the archive stands at the canonical destination with a manifest that reads back, names at least one member, and is the manifest the journal's own digest records; and every step the journal claims finished has left its evidence. None of that proves origin. All of it catches corruption, partial writes, stale artifacts and honest operator mistakes — which is what actually goes wrong, overwhelmingly more often than an attack does.

The second is INTEGRITY, and it is the real safety property: nothing is destroyed unless it is provably preserved. While the removal step is outstanding the contents are still in place, so every live file is hashed and required to equal the hash the manifest records for that same path, and every archived member is re-hashed and required to equal it too. The chain is live equals manifest equals archived, so the deletion proceeds only over bytes that demonstrably exist in the preserved copy. Comparing filenames would not do it — an archive holding the right paths and the wrong bytes would authorize destroying the real ones — which is why the comparison is on content. A mismatch refuses, including the honest case of a file edited while the run was interrupted: the archive is stale, the newer bytes are not in it, and finishing the run would destroy them. The remedy is to retire again from what is actually there, and the refusal says so.

That comparison runs at the DESTRUCTIVE BOUNDARY, not only when a run is admitted. Admission decides whether a run may proceed; it happens before the operator is asked to confirm, and a person takes as long as a person takes. In that interval an editor with the directory open, a build, or another terminal can write into the contents about to be destroyed, and anything appearing then is in no archive. So the full comparison — every live file hash-equal to its manifest member, no live path the manifest does not name, and every archived member still hashing to what the manifest records — is performed again immediately before the removal, on every path that reaches it, fresh run and resume alike. Both halves are re-run because the archived side can move in the same window as the live side, and half a chain proves nothing. A refusal there aborts with the same stale-archive guidance and removes nothing.

The third is AUTHORIZATION, which is not the journal's to give. A resumed run passes the same person a fresh execution does: unattended it refuses without asking, and attended it proceeds only on an explicit confirmation. So the residual — a chain forged by someone who already has write access to the repository — reduces to a person being asked, at a terminal, to confirm destroying bytes that are verifiably archived and readable from the archive afterwards. That is a bounded and honest position, and it is a stronger thing to be able to say than a provenance claim this trust model cannot support.

Any failure refuses the whole run — including one aimed at a different root — rather than skipping the file, since a journal that does not check out is exactly the case where continuing is unsafe.
**Invariants**:
...
- A file in the journal location whose name is not a root name is refused rather than read.
- A journal recording a root other than the name it is filed under is refused, and nothing is deleted or deregistered on its authority.
- A journal recording a root that is not a plain slug, or a path that leaves the project, is refused.
- A journal carrying a key the shape does not define, or more than one document, is refused rather than decoded with the surplus dropped.
- A journal whose outstanding steps are empty, unknown, duplicated or out of the order they must happen in is refused.
- A journal with no archive at the canonical destination for its root, or whose archive has no manifest that reads back and verifies, is refused rather than resumed — so a hand-written journal cannot retire a live root.
- A journal that fails authentication refuses the run even when the run was aimed at a different root, and leaves the project unchanged.
- A journal recording no manifest digest, or a digest other than the one the archive's manifest file carries, is refused — a manifest rewritten consistently under a journal no longer matches it.
- The archived members are re-read and re-hashed against the manifest before the journal is acted on, so archived bytes that changed while the manifest did not are caught.
- A manifest naming no members is refused as not being a root's archive.
- A step the journal claims finished must have left its evidence: a finished record step requires a retirement record beside the archive naming the same root and the same path.
- While the root's contents are still in place, every live file must hash to the value the manifest records for it, so the run destroys contents only once every one of them is provably in the preserved copy; comparing names alone is not sufficient and is not what happens.
- That comparison is performed again immediately before the removal itself, on every path that reaches it, and not only when the run was admitted — a file written or edited while the confirmation was waiting is content no archive holds.
- Content that appears or changes between authorization and the removal refuses the run, naming the file and the stale-archive remedy, and nothing is removed or deregistered.
- The archived half is re-verified at that same boundary, so an archive altered while the confirmation was waiting aborts the removal too.
- A live file holding different bytes from its archived copy refuses the run, including when the difference is an honest edit made while the run was interrupted, and the refusal says the archive is stale and the retirement should be run again from what is there.
- A live file the archive does not name at all refuses the run.
- A forged chain naming every live path but holding different bytes — with the manifest, its digest, the record and the journal all recomputed to agree — is refused, because what is checked is the content that would be destroyed rather than the shape of the artifacts.
- A forged chain whose archive holds the root's actual bytes satisfies every check, and is harmless on those terms: it has preserved what it proposes to remove, the removal still passes a person, and every destroyed byte is readable in the archive afterwards. Authenticity is not established by any of this and is not claimed.
- A resumed run passes the same authorization a fresh execution does: unattended it refuses without asking, and attended it proceeds only on an explicit confirmation; a preview of a part-finished run remains available unattended and commits to nothing.
- A journal this operation actually wrote authenticates and resumes, and the digest it records survives a completed step.
