...
Because a journal is found by scanning a location rather than by being handed over, it must be AUTHENTICATED before it is acted on. What a resumed run does on its authority is delete a directory and remove a registration, with no preflight, no sweep and no disposition record — correctly, since the run that wrote the journal performed all of those. That makes the file a delegation of destructive authority, and a file in a known location with a known extension is the easiest thing in the project to produce. So the operation establishes that the file is its own record: the decode is closed, so an unknown key is refused rather than dropped; the recorded root is a plain slug and equals the name the file is filed under, since a journal claiming one root while filed under another is claiming authority over a retirement it is not the record of; the recorded path is contained; the outstanding steps are a non-empty tail of the canonical order, which is the only shape the writer produces and which rules out unknown, duplicated, reordered and post-terminal steps together; and the archive the journal is the tail of stands at the canonical destination for that name with a manifest that reads back and verifies. That last check is the load-bearing one, because writing a file is trivial and producing a verified archive is not. Any failure refuses the whole run — including one aimed at a different root — rather than skipping the file, since a journal that cannot be authenticated is exactly the case where continuing is unsafe.

Shape is not provenance, and the distinction is the whole of the risk. A manifest's self-covering hash establishes that its member list is internally consistent, which a list invented from nothing satisfies perfectly; a journal's step order establishes that it is well formed. Neither says the two came from a run that archived this root. Three further checks close that gap, each on its own terms. The archived members are re-read and re-hashed against the manifest, so the manifest describes files rather than only itself, and an archive naming no members at all is refused outright — a root preserves at least its own configuration, and an empty member list is the cheapest forgery there is. The journal names the archive it belongs to by the digest of the manifest FILE, recorded when the journal was written: a manifest can be rewritten and re-cover its own list, and only a value held somewhere else notices. And the progress the journal claims is cross-checked against the filesystem, because the outstanding list is an assertion about what has already happened: a step claimed finished must have left its evidence, and a step not yet taken must not yet have had its effect.

The last of those is the one that survives a determined forgery. Someone who can write files can write a consistent archive, hash its manifest and record that digest in a journal. What they cannot do without genuinely archiving the root is produce a copy that already accounts for every file the root's directory still holds — and while the removal step is outstanding, those files are by construction still there. A retirement deletes contents only in favour of a copy of exactly those contents, so an archive that does not cover them is not this root's archive, whatever else about it is well formed. The retirement is complete only once the registration no longer names the root and no journal outlives it.
**Invariants**:
...
- While the root's contents are still in place, the archive must already name every file they hold, so an archive that is not this root's cannot authorize deleting it.
- A well-formed forged archive and journal — consistent contents, a verifying manifest, a correct digest and a well-ordered step tail — cannot delete or deregister a live root.
