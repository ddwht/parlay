...
**Goal**: Each non-presentation adapter declares the closed-vocabulary terms it implements — operation kinds, steps, errors, and policies — so build validation can fail *before the AI is invoked* when a feature requires terms no selected adapter supports. Presentation adapters keep their existing surface-vocabulary contract; backend adapters add the new declarations.

**Persona**: Parlay tool maintainer

**Priority**: P0

**Context**: AI-driven codegen has no useful failure mode for "your prisma adapter does not implement `read-tree`" — it will improvise something that compiles and passes shallow tests. The cure is to refuse generation up front. Each adapter therefore enumerates which terms from the closed backend vocabulary it implements; build-feature fails when the resolved feature uses a term outside the registered adapter's `supports`. The pattern descriptions for each supported term live alongside `supports:` and feed the AI prompt at generation time, but the *gate itself* is mechanical.

**Action**: Add a `supports:` block to the adapter-file schema with sub-keys `operation_kinds`, `steps`, `policies`, and `errors`, each a list drawn from the closed backend vocabulary. During build-feature, for every operation declared in the resolved `capabilities.yaml`, the validator checks (operation kind, every step type, every policy, every error) against the `supports:` of the adapter occupying the relevant slot. Any unsupported term fails build with a stable error code naming the term, the term kind, the adapter, and the operation that surfaced it. Pattern descriptions for supported terms are authored alongside `supports:` in the same adapter file; they are AI prompt material, not validator input.
