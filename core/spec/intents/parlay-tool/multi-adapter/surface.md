...
**Notes**:
- Output for `parlay build-feature` when a feature's resolved capability operations use a term that no filled backend adapter's `supports:` block covers (union coverage across backend slots). Fails before any AI invocation.
...
- `message` names the operation, the term, the term kind, and the filled backend slots — and offers two fix paths: remove the term from this operation, or add/swap a backend adapter whose layer supports it.
