...
**Behavior**: Add a `supports:` block to the adapter file schema for non-presentation kinds, with sub-keys `operation_kinds`, `steps`, `policies`, and `errors` — each a list drawn from the corresponding closed vocabulary. An adapter lists **only the terms its own layer implements** (a persistence adapter lists the data steps and the transaction policy; an application adapter lists `validate-input`/`authorize`/`return-*` and the auth policies) — never terms another layer owns. During build-feature the gate has two parts: (a) a **per-adapter** shape/vocabulary check, and (b) a project-wide **union coverage** check — for every operation declared in the resolved `capabilities.yaml`, each term (kind, step, policy, error) must be supported by **at least one** filled backend adapter. A term fails only when **no** filled backend layer owns it (e.g. a `create-one` step in a project with no persistence slot). Fail the build before any AI invocation. Pattern descriptions live alongside `supports:` in the adapter file but are AI prompt material, not validator input.
**Invariants**:
- An adapter declaring a `supports.*` term that is not in the corresponding closed vocabulary fails with `adapter-supports-unknown-term` (per-adapter)
- A presentation adapter declaring a non-presentation `supports:` shape (e.g. `supports.steps`) fails with `adapter-supports-shape-mismatch` (per-adapter)
- A feature whose resolved operation uses a term supported by **no** filled backend adapter fails build with one of `adapter-supports-missing-operation-kind`, `adapter-supports-missing-step`, `adapter-supports-missing-policy`, `adapter-supports-missing-error` (union coverage — a term one backend layer owns passes even though the other backend adapter does not list it)
...
**Behavior**: Pin blueprint's owned scope to `app`, `shells`, `navigation`, `authorization`, `data`, `errors`, `state`, and `platform` — the list `blueprint.schema.md`'s "Owned scope" section documents, which is the single source and is tied to the validator's map by a conformance test. Reject any `targets:` block in blueprint (topology is owned by `.parlay/adapter-set.yaml`). Resolve any layered setting through the precedence `blueprint > adapter-set > adapter default`. Validate strategy choices against the relevant adapter's declared support, **at the key paths and vocabularies the schema body documents**: `data.fetching: {on-mount, prefetch, stale-while-revalidate, graphql, none}`, `data.caching.strategy: {none, in-memory, local-storage, service-worker}`, `data.offline.strategy: {none, read-only-cache, optimistic-writes}`, `authorization.strategy: {role-based, permission-based, attribute-based, none}`, `errors.retry.strategy: {none, exponential-backoff, immediate-once}`, `errors.retry.applies-to: {reads, writes, all}`. Walk every canonical operation error and confirm a mapping exists at adapter, adapter-set, or blueprint level; fail with `error-no-mapping` otherwise.

This paragraph previously named four settings that do not exist as written, and the implementation followed it faithfully. It said `auth.strategy: {none, session, jwt, oauth2}` — an authentication mechanism, where the schema defines `authorization.strategy` as an authorization model; and `errors.retry: {none, reads, writes, all}`, which is the vocabulary of the sibling key `errors.retry.applies-to`, so it would have rejected every legal retry strategy. It also flattened `data.caching` to a scalar when the schema documents a map. The resulting gate decoded shapes no real blueprint has, failed its unmarshal on any populated file, and skipped every strategy check in silence. A spec that names a key path wrongly does not produce a validator that is merely too strict — it produces one that never runs.
**Invariants**:
...
**Invariants**:
- `spec/intents/**` is off-limits to codegen. This is an **agent-enforced obligation**, not a tool-enforced gate: parlay performs none of codegen's file access — the agent reads with its own tools — so nothing in the CLI can observe a read attempt, let alone abort on one. The earlier wording promised `codegen-spec-read-forbidden` would fire before any output was written; no code could have emitted it, and the Go predicate written for it was deleted rather than left to look like a gate. Reads leave no artifact, so unlike the write-set there is nothing to audit after the fact either — auditing reads would require the agent to self-report a read log, which is a guard trusting its own subject
- Each adapter reads only under its target's declared root. Same enforcement status as the line above: an obligation on the agent, unobservable by the tool. `codegen-input-out-of-scope` is likewise not emitted by any code
