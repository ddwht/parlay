...
func TestNewInitiative_CreatesThreeTreeDirs(t *testing.T) {
	...
	os.MkdirAll(testContext(t).BuildRoot(), 0755)

	err := runNewInitiative(testCommandWithContext(t, testContext(t)), []string{"auth", "overhaul"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		...
		filepath.Join(testContext(t).BuildRoot(), "auth-overhaul"),
	} {
...
func TestNewInitiative_IdempotentSecondRun(t *testing.T) {
	...
	runNewInitiative(testCommandWithContext(t, testContext(t)), []string{"auth", "overhaul"})
	err := runNewInitiative(testCommandWithContext(t, testContext(t)), []string{"auth", "overhaul"})
...
func TestNewInitiative_RejectsOrphanFeatureCollision(t *testing.T) {
	...
	err := runNewInitiative(testCommandWithContext(t, testContext(t)), []string{"password-reset"})
