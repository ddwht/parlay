...
func SaveRootsIndex(idx *RootsIndex) error {
	...
	// roots exist, so a .parlay replaced with a link out of the project
	// must not be able to carry this write outside, leaving the real
	// index untouched while the caller reports success. The relative
	// path is a fixed two-component constant, so the rooted handle can
	// only ever land on the project's own index.
